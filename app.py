...
model_C = joblib.load(open('model/classification.pkl','rb'))
model_R = joblib.load(open('model/regression.pkl','rb'))
...
def predict():
    try:
        ...
        if output == 0:
            text = 'Forest is Safe!'
            steps="\nKeep forest clean and make environment healthy. Forest is totaly safe this time .\nYou can enjoy in to the forest."
            steps = steps.splitlines()
        else:
            text = 'Forest is in Danger!'
            f=open("important_steps.txt",'r')
            # import pdb; pdb.set_trace();
            steps=f.read()
            steps = steps.splitlines()
            f.close()
        return render_template('index.html', prediction_text1="{} --- Chance of Fire is    {}\nImportant steps to take:\n".format(text, output), steps=steps)
    except Exception as e:
